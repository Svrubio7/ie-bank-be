...
def test_get_account(testing_client):
    ...
    """
    _ = testing_client.post('/accounts', json={'name': 'Juan Devops', 'currency': '€', 'country':'Mozambique'})
    
    account = Account.query.filter_by(name='Juan Devops')[0]
...
def test_create_account(testing_client):
    ...
    THEN check the response is valid
    """
    response = testing_client.post('/accounts', json={'name': 'Juan Devops', 'currency': '€', 'country':'Mozambique'})
    assert response.status_code == 200
...
def test_delete_account(testing_client):
    ...
    accounts = Account.query.filter_by(name='Juan Devops').all()
    assert accounts == []
...
def test_update_account(testing_client):
    ...
    response = testing_client.put(f'/accounts/{account.id}', json={'name': 'Roro', 'country': 'Venecolandia'})
    account = Account.query.get(account.id)
    
    
    assert response.status_code == 200
    assert account.name == 'Roro'
    assert account.country == 'Venecolandia'
...
def test_create_account_data(testing_client):
    ...
    assert account.name == 'Juan Devops'
    assert account.currency == '€'
    assert account.account_number != None
    assert account.balance == 0.0
    assert account.status == 'Active'
    assert account.country == 'Mozambique'
